...
let unsorted, sorted, unsortedNegative, sortedNegative;
...
describe("#mergeSort", function () {
  describe("#merge", function () {
    it("should merge two sorted arrays into one sorted array", function () {
      expect(merge([1, 3, 5, 7], [-2, 4, 6, 8])).to.deep.equal([
        -2,
        1,
        3,
        4,
...
describe("#quickSort", function () {
  describe("#pivot", function () {
    ...
    it("should mutate the input so that the pivot is in the right place", function () {
      ...
      expect(unsortedNegative.slice(0, 4)).to.include.members(
        sortedNegative.slice(0, 4)
      );
      expect(unsortedNegative.slice(5)).to.include.members(
        sortedNegative.slice(5)
      );
    });
  });
